...
화면은 세 덩어리다. 44px 상단 바, 접었다 펴는 왼쪽 사이드바(248px), 그리고 가운데의 복셀 스튜디오 + 터미널. 상태는 상단 바에서 한눈에 보이고, 자세한 건 전부 그 칩을 눌러 여는 팝오버 안에 있다.

**테마는 밝은 단색 하나다**(다크 모드도 토글도 없다). 종이 같은 흰 표면(`--surface`) 위에 회녹색 배경(`--bg`)과 깊은 숲 초록 강조색(`--accent #2f6b4f`) 하나, 상태색은 주의(`--warn`)·경고(`--danger`)·정보(`--info`) 셋뿐이다. 본문 글자는 자기 표면에서 최소 4.5:1(실측 `--text` 16.7:1 · `--text-2` 6.6:1 · `--text-3` 4.8:1)이고, 색만으로 상태를 말하는 곳은 없다 — 게이지에는 늘 숫자가 붙는다. 타이포는 11/12/13/15px 네 단만 쓰고, 숫자는 `tnum` 으로 폭을 고정해 퍼센트가 떨릴 일이 없다. 폰트는 CSP(`font-src 'self' data:`)가 외부 폰트를 막으므로 시스템 폰트(Pretendard → system-ui → Malgun Gothic)만 쓴다.

**딱 한 곳, 터미널만 어둡다**(`--term-bg #121814`). Claude Code 가 그 안을 자기 팔레트로 직접 칠하기 때문에 배경을 밝게 하면 CLI 가 쓰는 흐린 색이 전부 사라진다. 3D 캔버스 위에 뜨는 오버레이도 밝은 모래·바다 위에 얹히므로, 흰 패널마다 hairline(`#17201a1f`)과 그림자를 줘서 배경에 묻히지 않게 한다.
...
- 오른쪽 상태 칩(순서 고정, 전부 팝오버):
  ...
  - **⋯**(점 세 개): `사이드바 (Ctrl+B)` · `책상 펼치기` · `바뀐 파일 패널` · `항상 위` 체크 항목, **말풍선** 의 `요약해서 말하기`(요약기를 쓸 수 없으면 비활성 + 이유)와 언어(자동/한국어/English/日本語/中文/…), **Claude Code** 의 현재 버전·`다시 확인`·업데이트 설치.

**사이드바** (사이드바 아이콘 또는 `Ctrl+B`, 기본 숨김)
...
- 맨 위 `프로젝트·폴더 검색` 한 칸이 두 섹션을 같이 거른다. 목록 행은 좌우 6px 마진 + 라운드로 카드처럼 떨어져 있고, 폴더 아이콘은 `.git` 이면 branch, `CLAUDE.md`/`.claude` 면 🐹 다.
...
- **앱에서 유일하게 어두운 패널**이다(`--term-bg #121814`, 전경 `#dfe6da`, 커서 `#7fd4a3`). Claude Code 가 자기 테마 색으로 출력하므로 여기만 밝게 하면 글자가 안 보인다.
...
- **자동 카메라**(`자동` 버튼, 기본 켜짐): 12석 사무실 전체가 아니라 **지금 있는 햄스터들**만 화면에 담는다. 매 프레임 앉은 햄스터는 좌석 + 앞 책상 한 타일, 걷는 햄스터는 현재 위치를 모아(각 점 ±80) `frameCamera` 로 목표 구도를 구하고, `tx/tz/배율`을 지수 보간(`1 − e^(−6·dt)`)으로 따라간다 — 스냅하지 않으므로 동료가 들어오면 시야가 부드럽게 넓어진다. 목표는 점유 집합이 바뀌거나 누가 걷는 동안만 다시 계산한다. 배율은 250%(한 마리 = 책상 근접)와 60%(12석 전부가 보통 뷰포트에 들어오는 값) 사이로 묶인다. 메인만 있을 때는 정확히 `⌂` 와 같은 사장 자리 250% 구도라 처음 열릴 때 그 화면으로 시작하고, 직원 책상은 앞쪽(남쪽) 전경에 깔린다. 드래그·휠·회전·`+`/`−`·미니맵 클릭·`전체 보기`·`동료 위치 찾기`·더블클릭 등 **수동 조작은 자동을 끄고**, `⌂` 가 다시 켠다. 자동 여부는 카메라와 함께 세션별 씬에 저장되므로 책상을 접었다 펴거나 탭을 오가도 유지된다.
- **햄스터는 게임 캐릭터 그대로**(`src/desk/vox/hamster.ts`): `imjustawall_boxel` 의 `buildCat(kind, material, paint, biped = true)` 지오메트리와 조립(몸통·머리·귀·속귀·눈·주둥이·코·팔다리 공용 지오메트리·꼬리, 좌표·크기·회전·그룹 위치)을 1:1 로 옮겼다. 네발 좌표로 만든 몸통을 통째로 세워(`rotation.x = -π/2`) 등·머리 방향이 저절로 맞고, 팔 2·다리 2 는 게임의 네 다리처럼 한 지오메트리를 공유한다. 앉기/타이핑/읽기/생각/통화/인사/잠 포즈는 절차적으로 만든다. 팔레트는 털·등줄무늬만 모델 스킨이 정하고 배·눈(`0x252a33`)·코(`0xd97a8a`)·속귀는 게임 값이다. 목도리는 없고 **배 앞에 작은 넥타이**(매듭·몸통·끝 세 상자, 가슴 패널 앞 0.1 에 떠서 겹면 없음)가 붙는데 그 색이 에이전트 종류(`tintFor`)다. 머리 악세서리(왕관·안경·헤드폰·잎)는 귀 앞쪽 두개골 위에 얹혀 귀와 겹치지 않는다. 책상 앞판이 낮아 앉으면 머리와 팔이 상판 위로 올라온다.
...
- 말풍선 DOM 요소는 **햄스터당 하나**(`key = bubble:<세션>:<햄스터>`)로 유지되고, 새 문장은 안쪽 `.ob-speech` 만 문장별 key 로 다시 마운트해 팝인을 재생한다. 요소를 문장마다 갈아 끼우면 렌더 루프가 위치·투명도를 쓰는 `tags` 맵에서 옛 요소의 ref 정리가 새 요소 등록 뒤에 실행될 수 있고, 그러면 새 말풍선이 마운트 시의 `opacity: 0` 에 갇혀 ✓ 를 누르기 전까지 안 보였다. ref 정리는 자기 자신이 등록돼 있을 때만 맵에서 지운다. 회귀 케이스는 `test:office:ui` 에 있다(`window.__studio.say` 로 300ms 간격 두 문장 → 말풍선 하나·두 번째 문장·opacity 1 → ✓ 로 제거).
...
**오른쪽 "바뀐 파일"** (300px, 기본 숨김; 상단 `바뀐 파일 N` 칩이나 `⋯` 메뉴로 연다): 파일별 횟수·줄 수·누가·언제. 헤더는 상단 바와 같은 44px 라 두 줄이 맞아떨어진다. 클릭하면 마지막 Edit 의 old/new 미리보기(지운 줄 붉은 바탕, 넣은 줄 초록 바탕). 헤더의 닫기 아이콘으로 닫는다.
...
src/App.tsx             상단 바·패널 배치 · src/widgets/ Popover(공용)·사용량 게이지 둘·버전·⋯ 메뉴·+ 메뉴·icons.tsx(인라인 SVG 아이콘 한 벌)
src/styles.css          앱 전체의 라이트 테마 토큰과 모든 크롬 스타일(다크는 터미널 패널 하나뿐)
...
```bash
npm run typecheck
npm run test:office       # 좌석·경로·카메라·자동 프레이밍·햄스터 리그·섬 생성 회귀 테스트 13개
npm run build
...
npm run shot:studio      # 시각 검수용 장면 11컷 → work/shot-*.png
```

`test:office` 는 three 지오메트리만 쓰는 순수 모듈(`vox/*`, `office-camera`)을 node 에서 그대로 돌린다: 좌석 재사용·복도 경로(사장 자리 포함), 사장 자리가 slot 0 이고 에이전트가 절대 앉지 않는지, 줌이 커서 아래 지면점을 1e-6 이내로 유지하는지, `전체 보기`가 섬 네 귀퉁이를 뷰포트 안에 넣는지, `frameCamera` 가 한 마리면 250% 로 좌석을 가운데 두고 12석 전부면 모두 뷰포트 안이거나 하한 60% 에 멈추는지, 스킨 5종의 햄스터가 바닥에 서고 팔다리가 지오메트리를 공유하는지, 섬 생성이 두 번 호출해도 같은 결과인지(사무실 타일은 전부 높이 24, 나무는 사무실 밖).

`test:office:ui` 는 소프트웨어 WebGL(`enable-unsafe-swiftshader`)로 실제 렌더러를 띄워 컨텍스트 생성, 복셀 리그 구성(넥타이 그룹), 말풍선 교체 회귀(같은 햄스터에 두 문장 → 요소 하나·두 번째 문장·opacity 1 → ✓ 로 제거), 인원 증감·재정렬 후 명패 위치 고정, 드래그·미니맵·휠·확대·전체 보기, 동료 찾기(자동 카메라 해제), 세션 전환 및 접기/펴기, 작은 창, 48마리일 때 대기석·`동료 12 / 12` 표시, 빈 사무실과 렌더러 오류를 확인한다. 스튜디오 데모는 실제 셸이나 세션 감시기를 시작하지 않으며 저장된 설정을 덮어쓰지 않는다. 스크린샷(`work/studio-default.png`, `studio-overview.png`, `studio-compact.png`)과 결과 JSON은 무시되는 `work/`에 저장한다.

`shot:studio` 는 판정 대신 눈으로 볼 장면만 찍는다(`?studio-demo` 미리보기에서만 노출되는 `window.__studio` 훅 — `focus`·`zoom`·`orbit`·`setStates`·`addArriving`·`say`·`autoFrame` — 으로 카메라·상태를 조작한다): `shot-main-focus`(사장 자리 250%) · `shot-main-400`(메인 착석 근접) · `shot-back-view`·`shot-side-view`(뒤·옆에서 본 착석) · `shot-states-300`(8가지 상태의 포즈·글리프·화면색·상태등) · `shot-walk-a`/`walk-b`(복도를 걸어 들어오는 새 동료) · `shot-overview`(섬 전체) · `shot-autoframe-8`(자동 카메라가 9마리를 담은 구도) · `shot-autoframe-4`(사장 주변 세 자리만 찼을 때의 촘촘한 구도) · `shot-autoframe-1`(메인만 남았을 때 사장 자리로 돌아온 구도).
