...
- 검증(2026-09-19, 프롬프트 v1 기준): "README 끝에 한 줄 추가" 요청에 설계 담당은 읽기만 하고 `Agent(subagent_type: hd-implementer)` 로 명세를 넘겼고, 구현 담당은 `claude-opus-5` 로 실행돼 파일을 고치고 sha256 검증까지 보고했다.
- 프롬프트 v2(2026-09-20)에서 세 가지가 바뀌었다.
  - **설계 담당도 사소한 수정은 직접 한다**: 한 파일에서 합쳐 3줄 이하이고 설계 판단이 필요 없는 것(오타·문구·상수·한 줄 버그·import)만 `Edit` 로 고치고 타입 검사로 확인한다. 그래서 frontmatter 가 `disallowedTools: Write, NotebookEdit` 로 줄었다(Write·NotebookEdit 는 여전히 금지). 새 파일·여러 파일·로직 변경은 그대로 위임이고, 애매하면 위임이다.
  - **검증 기본값을 줄였다**: 명세의 검수 기준도 구현 담당의 검증도 기본은 `타입 검사 + 빌드 + 바뀐 영역의 기존 테스트` 다. Electron 스모크·스크린샷·실제 모델 호출·e2e 는 명세가 명시할 때만 돈다(시간과 사용자의 토큰을 쓰니까). 구현 담당은 보고에 **하지 않은 검증**도 적는다.
  - **위임 알림**: 구현 담당의 진행은 화면에 스트리밍되지 않으므로, 설계 담당이 위임 직후 무엇을 맡겼는지와 대략의 규모를 한두 줄로 알린다.
- 에이전트 파일에는 프롬프트 버전 마크(`<!-- generated by Hamster Desk collaboration mode · prompt v2 -->`)가 들어 있고, 협업 모드가 켜져 있는데 디스크의 파일이 구버전이면 앱을 켤 때 `refreshHarnessFiles()` 가 두 파일을 새 프롬프트로 다시 쓴다(꺼져 있으면 건드리지 않는다).
