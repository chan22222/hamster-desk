...
- 오른쪽 상태 칩(순서 고정, 전부 팝오버):
  ...
  - **⋯**(점 세 개): `사이드바 (Ctrl+B)` · `책상 펼치기` · `바뀐 파일` · `항상 위` · `미니 모드 (Ctrl+Shift+M)` 체크 항목, **계정**(전환·추가·이름 바꾸기·목록에서 빼기 — 아래 "여러 계정"), **알림** 의 `권한 요청`·`질문`·`턴 완료`·`소리`, `터미널 글꼴`(12·13·14·16), `책상 위치`(위·오른쪽)와 `테마`(라이트·다크·시스템) 라디오, **말풍선** 의 `요약해서 말하기`(요약기를 쓸 수 없으면 비활성 + 이유)와 언어(자동/한국어/English/日本語/中文/…), **Claude Code** 의 현재 버전·`다시 확인`·업데이트 설치, 그리고 맨 아래 11px 로 `설정 파일: ~\.hamster-desk\ui.json`(누르면 탐색기에서 열린다).
...
- **추가**: `⋯ > 계정 > 계정 추가` → 이름(예: `회사`) → 그 계정이 바로 현재 계정이 된다. 새 터미널을 열어 `claude` 를 실행하고 **`/login` 한 번**이면 끝이다. 로그인한 뒤에는 목록에 그 계정의 이메일이 보인다(CLI 가 계정 폴더에 적어 둔 `.claude.json` 을 읽기만 한다).
...
- **목록에서 빼기**(행의 `×`)는 앱이 그 계정을 잊는 것뿐이다 — 폴더(로그인·대화 기록)는 **지우지 않는다**. 기본 계정은 뺄 수 없고 이름만 바꿀 수 있다. 행의 폴더 아이콘은 그 계정 폴더를 탐색기로 연다.
