...
- 방은 처음부터 고정 크기이고 햄스터 수에 따라 바뀌지 않는다. **북쪽 벽 앞에 사장 자리 하나**(`OFFICE.slots[0]`: 2.5타일짜리 호두나무 책상, 모니터 2대, 스탠드, 등받이 높은 의자, 러그, 양옆 화분)가 따로 있고, 세 타일 아래로 **직원 책상 12개(4열×3줄)** 가 깔린다. 메인 햄스터는 언제나 사장 자리, 서브에이전트는 직원 자리만 쓴다(`reconcileSeats`: `main → 0`, 에이전트는 1 이상). 자리는 빈자리만 재사용하므로 동료가 늘거나 줄어도 이미 앉은 햄스터는 움직이지 않는다. 상단의 `동료 N / 12` 는 사장을 뺀 직원 자리 점유다. 12마리를 넘으면 문 옆에서 "빈자리 대기". 사장 자리도 다른 자리처럼 왼쪽 복도(i=0.5)를 타고 북쪽 통로(j=1.25)로 걸어 들어간다.
...
src/App.tsx             상단 바·패널 배치 · src/widgets/ Popover(공용)·사용량·버전·⋯ 메뉴·+ 메뉴
...
npm run shot:studio      # 시각 검수용 장면 10컷 → work/shot-*.png
```
