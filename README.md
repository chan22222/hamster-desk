...
- **자동 카메라**(`자동` 버튼, **기본 켜짐**): 12석 사무실 전체가 아니라 **지금 있는 햄스터들**만 화면에 담는다. 매 프레임 앉은 햄스터는 좌석 + 앞 책상 한 타일, 걷는 햄스터는 현재 위치를 모아(각 점 ±44) `frameCamera` 로 목표 구도를 구하고, `tx/tz/배율`을 지수 보간(`1 − e^(−6·dt)`)으로 따라간다 — 스냅하지 않으므로 동료가 들어오면 시야가 부드럽게 넓어진다. 목표는 점유 집합이 바뀌거나 누가 걷는 동안만 다시 계산한다. 배율은 340%(근접)와 75%(사무실이 꽉 찼을 때의 하한) 사이로 묶인다.
  ...
  - 한 마리만 있을 때는 `focusCamera(…, 3.0, dy 40)` 로 300% 책상 근접 — `⌂` 의 250%(`focusCamera` 기본값, `동료 위치 찾기`와 테스트가 쓰는 값)보다 가깝다.
...
```bash
npm run typecheck
npm run test:office       # 좌석·경로·카메라·자동 프레이밍·햄스터 리그·섬 생성 회귀 테스트 15개
npm run build
