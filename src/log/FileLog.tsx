 ...
 */
const whoOf = (e: EditEntry, mainName: string): string => (e.who === 'main' ? mainName : e.whoName)

/** One row per file the session touched, newest first. */
export function changedFiles(session: SessionState | null, mainName = ui().common.mainHamster): FileAgg[] {
...
export function FileLog({ session }: { session: SessionState | null }) {
  ...
  const files = useMemo(() => changedFiles(session, u.common.mainHamster), [session, u])
            ...
            <div className="file-preview">
              <div className="pv-head">
                <span className="pv-label">
                  {f.last.op === 'write' ? u.files.lastWrite : u.files.lastEdit} · {whoOf(f.last, u.common.mainHamster)} · {timeOf(f.last.ts)}
                </span>
