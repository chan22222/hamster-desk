...
test('add → rename → setCurrent → remove, all inside HAMSTER_HOME', () => {
  ...
  assert.equal(gone.currentId, 'default', 'forgetting the current account falls back to the default one')
  assert.ok(existsSync(added.dir as string), 'forgetting an account never deletes its folder')
  assert.equal(removeProfile('default').list.length, 1, 'the default account cannot be removed')
