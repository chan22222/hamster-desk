...
- **지난 대화**는 모든 계정의 기록을 한 목록에 모아 보여 준다(최신순). 대화는 계정 폴더마다 따로 쌓이고 계정을 오가며 쓰기 마련이라, 예전처럼 그 탭 계정의 기록만 보여 주면 어제의 대화가 다른 계정에 있어서 안 보이고 몇 주 전 것만 나왔다. 탭 계정이 아닌 행에는 그 계정의 배지가 붙고, 고르면 **그 계정의 새 탭**에서 이어진다 — `claude --resume` 은 자기 계정 폴더에서만 찾기 때문이다(파일을 다른 계정으로 옮기지 않는다). 탭 계정의 행은 예전처럼 같은 계정에서 이어진다.
...
- 복사·붙여넣기는 Windows Terminal 과 같은 규칙이다. **선택이 있으면 `Ctrl+C` 가 복사**(선택이 풀리므로 한 번 더 누르면 평소처럼 `^C` 인터럽트), 선택이 없으면 그대로 `^C`. `Ctrl+Shift+C`·`Ctrl+Insert` 는 언제나 복사, `Ctrl+V`·`Ctrl+Shift+V`·`Shift+Insert` 는 붙여넣기(bracketed paste 라 Claude Code 가 한 번의 붙여넣기로 인식한다). **우클릭**은 선택이 있으면 복사, 없으면 붙여넣기 — 단, 터미널 안의 프로그램이 마우스를 잡고 있으면(Claude Code, `mouse=a` 인 vim) 그 클릭은 프로그램의 것이라 앱은 손대지 않는다(Windows Terminal 과 같다; [설계 노트](design-notes.md#붙여넣기는-한-번만-거른-다음에)). Claude Code 는 우클릭에 자기 선택을 복사하거나, 선택이 없으면 스스로 클립보드를 읽어 붙여넣는다. `Shift+우클릭` 은 xterm 이 잡아 두므로 언제나 앱의 복사·붙여넣기다. 붙여넣는 글은 Windows Terminal 처럼 먼저 거른다: 탭·줄바꿈 말고는 제어 문자(ESC 포함)를 모두 지워서, 클립보드에 숨은 `ESC[201~` 가 붙여넣기를 일찍 끝내고 그 뒤의 명령을 실행시키지 못한다(`src/term/paste.ts`, [설계 노트](design-notes.md#붙여넣기는-한-번만-거른-다음에)).
...
- 그 탭 폴더에서 **모든 계정으로** 나눈 대화 목록이다: 제목 / 마지막 프롬프트 / `3시간 전` · `⎇ branch` · `실행 중` · (다른 계정의 대화면 오른쪽에 계정 배지). 3개 이상이면 검색 칸이 생긴다. 행을 누르면 **새 터미널 탭**에서 `claude --resume <sessionId>`, 맨 아래 줄은 `claude --continue`(이 탭 계정의 마지막 대화). 계정 배지가 붙은 행은 시작 카드에서 골라도 그 터미널이 아니라 **그 계정의 새 탭**에서 열린다 — 그 터미널의 셸은 다른 계정으로 떠 있다. 이미 돌고 있는 대화는 `실행 중` 배지와 함께 비활성이다(앱 밖에서 같은 대화를 또 여는 것까지 막지는 못한다). 프롬프트가 하나뿐이라 제목과 부제가 같은 대화는 부제를 숨긴다.
- 목록은 **트랜스크립트 파일만** 읽는다(계정마다 `<설정 폴더>/projects/<cwd 슬러그>/*.jsonl` 을 모아 mtime 순으로 최근 60개; 같은 세션 id 가 두 계정에 있으면 새 쪽 하나). 파일 하나에서 읽는 양은 **앞 64KB**(첫 프롬프트·브랜치) + **꼬리 512KB**(마지막 `custom-title`/`ai-title`/`last-prompt`)뿐이고 전량 파싱은 하지 않는다 — 95MB 짜리 트랜스크립트도 있다. 제목 우선순위는 `custom-title > ai-title > 첫 프롬프트 60자 > id 앞 8자`. 직접 친 프롬프트가 없는 파일(슬래시 명령만 있는 세션 등)은 목록에서 빠진다. 결과는 `경로:크기:mtime` 으로 캐시한다(`electron/transcripts.ts`).
...
- 햄스터 1마리 = 메인(사장 자리). `Agent`/`Workflow` 로 서브에이전트가 뜨면 문으로 걸어 들어와 직원 자리에 앉고, 보고가 끝나면 **문까지 걸어 나간 뒤에** 사라진다 — 스토어는 퇴근 2.6초 뒤에 햄스터를 지우는데 먼 자리에서 문까지는 4.8초라, 예전엔 12자리 중 7자리에서 방 한가운데서 사라졌다. 이제 스튜디오가 문으로 가는 중인 걸음을 문에 닿을 때까지 그려 준다(`Scene.walkers`). 중단되거나 실패한 서브에이전트, 결과를 넘기고 끝나는 워크플로 에이전트도 퇴근한다 — 예전에는 "끝났다"는 표시(`end_turn`)를 남기지 않는 이런 에이전트가 세션이 끝날 때까지 일하는 모습으로 자리에 남았다. 넥타이 색 = 에이전트 종류.
